...
export default function NeighborDetails() {
    ...
    const [secondMember, setSecondMember] = useState<Member | null>(null);
    ...
    const [similarityScore, setSimilarityScore] = useState<string | number | null>(null);
    const [overlapCount, setOverlapCount] = useState<string | number | null>(null);
    const [avgRatingDistance, setAvgRatingDistance] = useState<string | number | null>(null);
    const [agreedFilms, setAgreedFilms] = useState<NeighborFilm[]>([]);
    const [agreedPage, setAgreedPage] = useState(1);
    const [differFilms, setDifferFilms] = useState<NeighborFilm[]>([]);
    const [differPage, setDifferPage] = useState(1);
    const filmsPerPage = 20;
    const [agreedTotalPages, setAgreedTotalPages] = useState<number | null>(null);
    const [differTotalPages, setDifferTotalPages] = useState<number | null>(null);
        ...
        const fetchNeighborAgreedFilms = async () => {
            try {
                ...
                if (rows.length === 0) {
                    ...
                    // total_count > 0
                    setAgreedTotalPages(Math.ceil(Number(rows[0].total_count) / filmsPerPage));
        ...
        const fetchNeighborDifferFilms = async () => {
            try {
                ...
                if (rows.length === 0) {
                    ...
                    // total_count > 0
                    setDifferTotalPages(Math.ceil(Number(rows[0].total_count) / filmsPerPage));
    ...
    const handleAgreedPageChange = (newPage: number) => {
        setAgreedPage(newPage);
    };

    const handleDifferPageChange = (newPage: number) => {
        setDifferPage(newPage);
    };

    const getSimilarityString = (similarity_score: number) => {
    ...
    const avgRatingString = (avg_rating: string) => {
        if (Number(avg_rating) > 0) {
            return `Average rating: ${avg_rating} / 5`;
        }
        return '';
    }

    const areString = (number: number) => {
        if (number === 1) {
            return 'is';
        }
        return 'are';
    ...
    const filmString = (number: number) => {
        if (number === 1) {
            return 'film';
        }
        return 'films';
    ...
    const thoseString = (number: number) => {
        if (number === 1) {
            return 'that 1';
        }
        return 'those';
        ...
        return (
            <div className="container">
                ...
                <div>
                    ...
                    <Pagination currentPage={differPage} totalPages={differTotalPages || 1} onPageChange={handleDifferPageChange} />
                </div>
                <div className="my-4 text-start">
                    <p>
                ...
                <div className="my-4 score-details">
                    Similarity Score:
                    <span>
                        <CircleProgress percentage={Math.round(Number(Number(similarityScore).toFixed(2)) * 100)} />
                    </span>
                </div>
                <div className="mt-4 mb-5 text-start">
                    <p>
                        According to their Letterboxd ratings, their tastes in film are {getSimilarityString(Number(Number(similarityScore).toFixed(2)) * 100)}
                    </p>
                </div>
            </div>
        );
        ...
        return (
            <div className="container">
                ...
                <div>
                    ...
                    <Pagination currentPage={agreedPage} totalPages={agreedTotalPages || 1} onPageChange={handleAgreedPageChange} />
                </div>
                <div className="my-4 text-start">
                    <p>
                        On average, their difference in ratings of films they have both seen is {Number(avgRatingDistance).toFixed(2)} stars.
