...
const FilmDetails = () => {
    ...
    if (film.current_rank && film.previous_rank) {
        const change = Number(film.previous_rank) - Number(film.current_rank);
        if (change > 0) {
    ...
    if (film.current_rank && Number(film.current_rank) <= 250) {
    ...
    const getStarSymbols = (rating: number) => {
    ...
    return (
        <div className="film-details container">
            ...
            {[5, 4.5, 4, 3.5, 3, 2.5, 2, 1.5, 1, 0.5].map(star => {
                ...
                const filteredRatings = ratings.filter(userRating => Math.round(Number(userRating.rating) * 10) / 10 === star);
