...
function pickAvatarUrl(member, preferLarge) {
...
async function downloadAvatar(username, url, suffix = '') {
...
export async function discoverMembers() {
    ...
    let count = 0;
    for await (const member of paginate('/members', { member: seedLid, memberRelationship: 'IsFollowing', perPage: '100' })) {
