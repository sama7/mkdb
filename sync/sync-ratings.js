...
async function applySlugChange(client, lid, oldSlug, newSlug) {
    try {
        await client.query(
...
async function upsertFilmStub(client, { lid, slug, title, year }) {
    ...
    try {
        ...
        if (r.rowCount > 0) return r.rows[0].film_id;
        console.warn(`[ratings] slug "${slug}" already mapped to a different LID, skipping ${lid}`);
        return null;
    }
}

async function upsertRating(client, { userId, filmId, rating }) {
    await client.query(
...
async function fetchMemberStats(lid) {
    try {
        return await apiRequest('GET', `/member/${encodeURIComponent(lid)}/statistics`);
    } catch (err) {
...
async function syncMemberRatings(member, newFilmIds) {
    const client = await pool.connect();
    let ingested = 0;
    try {
        for await (const item of paginate('/films', { member: member.letterboxd_id, memberRelationship: 'Rated', perPage: '100' })) {
            ...
            const wasNew = existing.rowCount === 0;
            const existingNeedsDetails = existing.rowCount > 0 && existing.rows[0].details_fetched_at == null;
...
export async function syncAllRatings() {
