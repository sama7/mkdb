...
function tmdbUrlFromLinks(links) {
    if (!Array.isArray(links)) return null;
    const tmdb = links.find((l) => l?.type === 'tmdb');
    return tmdb?.url || null;
}

function directorNamesFromContributions(contributions, fallback) {
    if (Array.isArray(contributions)) {
        const directors = contributions.find((c) => c?.type === 'Director');
        if (directors?.contributors) {
            const names = directors.contributors.map((c) => c.name).filter(Boolean);
            if (names.length) return names;
        }
    }
    if (Array.isArray(fallback)) return fallback.map((d) => d.name).filter(Boolean);
    return [];
}
...
async function fetchAndStoreFilm(film) {
    const detail = await apiRequest('GET', `/film/${encodeURIComponent(film.letterboxd_id)}`);
...
export async function syncNewFilms({ limit } = {}) {
